...
class session:
        ...
        for spot in self.places:
            if spot == self.impostor_spot:
                ...
                self.image_list.append(
                    ImageTk.PhotoImage(
                        self.group_images.pop(
                            random.choice(range(len(self.group_images)))
                        )
                    )
                )
                self.button_list.append(
    ...
    def choice(self, is_impostor):
        ...
        if not is_impostor:
            print(
                f"Not the impostor! impostor is {self.people.impostor}, you clicked on {self.people.group}"
            )
